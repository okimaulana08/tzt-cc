<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('posts.index') }}" class="text-sm text-indigo-600 hover:underline">&larr; Back</a>
                ...
                <form action="{{ route('posts.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('status') border-red-400 @enderror">
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <button type="submit"
                                x-data x-on:click="$el.disabled = true; $el.textContent = 'Creating…'; $el.closest('form').submit()"
                                class="px-6 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 transition disabled:opacity-60 disabled:cursor-not-allowed">
                            Create Post
                        </button>
                    </div>
                </form>
